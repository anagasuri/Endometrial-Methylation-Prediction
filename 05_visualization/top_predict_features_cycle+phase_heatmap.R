############################################################
# Heatmap of Top 2303 Predictive CpGs - Cycle Phase (ELBOW)
############################################################
...
RANKED_CPG_FILE <- "/mnt/efs/home/ubuntu/cycle_phase_top_cpgs/cycle_phase_ranked_cpg_coefficients.csv"
H5AD_FILE <- "/mnt/efs/home/ubuntu/M_smartsva_corrected_with_SH_annot_filtered.h5ad"
OUT_FILE <- file.path(OUT_DIR, "cycle_phase_top2303_heatmap_unforced.png")
...
############################################################
# Keep only proliferative/secretory samples
############################################################

keep_samples <- meta$sample_id[!is.na(meta$CyclePhase)]

meth_sub <- meth_sub[, colnames(meth_sub) %in% keep_samples, drop = FALSE]
...
annotation_col <- data.frame(
  CyclePhase = factor(meta$CyclePhase,
...
pheatmap(
  ...
  main = "Top 2303 Predictive CpGs - Cycle Phase",
  filename = OUT_FILE,
  width = 24,
  height = 20
)

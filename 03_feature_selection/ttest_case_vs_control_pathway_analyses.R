...
# T-test FDR-significant CpGs only
...
sig_file <- "/mnt/efs/home/ubuntu/endo-me_data/ttest_feature_selection_outputs/case_control/case_control_ttest_FDR_0.05_selected_cpgs_n5.csv"

all_file <- "/mnt/efs/home/ubuntu/endo-me_data/ttest_feature_selection_outputs/case_control/case_control_ttest_all_cpgs_with_FDR.csv"

out_dir <- "/mnt/efs/home/ubuntu/endo-me_data/ttest_feature_selection_outputs/ttest_pathway_analysis_outputs/case_control/FDR_ttest"

dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

cat("\n====================================================\n")
cat("Starting case/control t-test pathway analysis\n")
...
write.csv(
  gene_map,
  file.path(out_dir, "case_control_FDR_ttest_mapped_entrez_genes.csv"),
  row.names = FALSE
)
...
write.csv(
  go_res,
  file.path(out_dir, "case_control_FDR_ttest_GO_all_results.csv"),
  row.names = FALSE
)
...
write.csv(
  kegg_res,
  file.path(out_dir, "case_control_FDR_ttest_KEGG_all_results.csv"),
  row.names = FALSE
)
...
write.csv(
  reactome_res,
  file.path(out_dir, "case_control_FDR_ttest_Reactome_all_results.csv"),
  row.names = FALSE
)
...
go_fdr <- save_fdr_results(
  go_res,
  db_name = "GO",
  out_file = file.path(out_dir, "case_control_FDR_ttest_GO_FDR_significant.csv")
)

kegg_fdr <- save_fdr_results(
  kegg_res,
  db_name = "KEGG",
  out_file = file.path(out_dir, "case_control_FDR_ttest_KEGG_FDR_significant.csv")
)

reactome_fdr <- save_fdr_results(
  reactome_res,
  db_name = "Reactome",
  out_file = file.path(out_dir, "case_control_FDR_ttest_Reactome_FDR_significant.csv")
)
...
plot_fdr_pathways(
  go_res,
  db_name = "GO",
  out_file = file.path(out_dir, "case_control_FDR_ttest_GO_FDR_barplot.png")
)

plot_fdr_pathways(
  kegg_res,
  db_name = "KEGG",
  out_file = file.path(out_dir, "case_control_FDR_ttest_KEGG_FDR_barplot.png")
)

plot_fdr_pathways(
  reactome_res,
  db_name = "Reactome",
  out_file = file.path(out_dir, "case_control_FDR_ttest_Reactome_FDR_barplot.png")
)
...
cat("\n====================================================\n")
cat("Case/control FDR CpG pathway analysis complete.\n")
cat("End time:", as.character(Sys.time()), "\n")
